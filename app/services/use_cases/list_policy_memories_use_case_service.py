"""Cross-service use case service for listing Policy Memories."""
...
class ListPolicyMemoriesUseCaseService(
    ListPolicyMemoriesUseCaseServiceProtocol
):
    """在平级 Project 与 Policy Memory 服务之上协调列表查询。"""
    ...
    async def execute(
        ...
        *,
        ...
        try:
            ...
            page = await self._policy_memory_service.list_policies(
                user_id=user_id,
                project_id=project_id,
                limit=limit,
                cursor=cursor,
        ...
        except PolicyMemoryServiceError as exc:
            error = self._map_policy_error(exc)
            self._log_failure(error, started_at=started_at, fields=log_fields)
            raise error from exc
            ...
            logger.error(
                "Policy Memory query failed unexpectedly.",
                extra={
    ...
    def _map_project_error(
        ...
        error_code, error_reason = mapping.get(
            error.error_code,
            ("MEMORY_QUERY_FAILED", "Policy Memory 查询失败，请稍后重试。"),
        )
        return ListPolicyMemoriesUseCaseError(
    ...
    def _map_policy_error(
        error: PolicyMemoryServiceError,
    ) -> ListPolicyMemoriesUseCaseError:
        mapping = {
