...
SEARCH_RESPONSE = DocsSearchResponse(
    ...
    source_summary={
        "searched_sources": ["openai_api", "anthropic_api"],
    },
)
...
def test_run_normal_path_maps_docs_results_to_normalized_items() -> None:
    ...
    result = asyncio.run(
        tool.run(
            LlmsTxtDocsSearchToolRequest(
                ...
                source_names=[" openai_api ", "", "anthropic_api"],
    ...
    assert docs_client.last_query.source_names == ["openai_api", "anthropic_api"]
    ...
    tool = LlmsTxtDocsSearchTool(
        docs_search_client=FakeDocsSearchClient(
            DocsSearchResponse(
                ...
                source_summary={"searched_sources": ["openai_api"]},
            )
        )
    )
...
def test_run_returns_failed_when_adapter_raises() -> None:
    ...
    assert result.retrieval_trace["selected_sources"] == ["openai_api"]
