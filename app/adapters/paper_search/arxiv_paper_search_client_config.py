...
Typed runtime settings for arXiv paper search."""
    ...
    timeout_seconds: float = Field(default=30.0, gt=0, description="单次 arXiv search HTTP 请求的超时时间，单位秒。")
    ...
    rate_limit_backoff_seconds: float = Field(default=10.0, gt=0, description="arXiv 返回 429 且未提供有效 Retry-After 时使用的等待秒数。")
    max_retry_after_seconds: float = Field(default=30.0, gt=0, description="接受 provider Retry-After 时允许等待的最大秒数，避免单次请求无限挂起。")
    ...
    def from_env(cls) -> "ArxivPaperSearchClientConfig":
        ...
        return cls(
            ...
            rate_limit_backoff_seconds=float(
                os.getenv(
                    "ARXIV_PAPER_SEARCH_RATE_LIMIT_BACKOFF_SECONDS",
                    str(cls.model_fields["rate_limit_backoff_seconds"].default),
                )
            ),
            max_retry_after_seconds=float(
                os.getenv(
                    "ARXIV_PAPER_SEARCH_MAX_RETRY_AFTER_SECONDS",
                    str(cls.model_fields["max_retry_after_seconds"].default),
                )
            ),
